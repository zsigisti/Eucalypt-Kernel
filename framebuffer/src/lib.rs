...
impl RendererCell {
    pub const fn new() -> Self {
        Self { inner: UnsafeCell::new(None), lock: SpinLock::new() }
    }
...
impl ConsoleLine {
    ...
    #[inline] pub fn set_width(&mut self, width: usize) {
...
impl ScrollingTextRenderer {
    ...
    fn parse_psf(data: &[u8]) -> (usize, usize, usize, usize) {
        if data.len() >= 32 && &data[0..4] == b"\x72\xb5\x4a\x86" {
            ...
            return (header.width as usize, header.height as usize, header.bytesperglyph as usize, header.headersize as usize);
        }
...
impl LineWriter {
    #[inline]
    pub fn new() -> Self {
        unsafe { *core::ptr::addr_of_mut!(LINE_WRITER_POS) = 0; }
        Self
    }
...
impl core::fmt::Write for LineWriter {
    ...
    fn write_str(&mut self, s: &str) -> core::fmt::Result {
        unsafe {
            ...
            core::ptr::copy_nonoverlapping(bytes.as_ptr(), (*buf_ptr).as_mut_ptr().add(pos), to_copy);
            *pos_ptr += to_copy;
            Ok(())
        }
    }
