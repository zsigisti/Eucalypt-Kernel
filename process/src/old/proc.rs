...
use memory::{addr::PhysAddr, frame_allocator::FrameAllocator, hhdm::HHDM_OFFSET, paging::PageTable, vmm::VMM};
use vfs::*;
use crate::Process;
...
pub fn destroy_process(pid: u64) -> bool {
    unsafe {
        ...
        if let Some(mut process) = PROCESS_TABLE.processes[pid as usize].take() {
            ...
            if !process.pml4.is_null() {
                let pml4_virt = (process.pml4 as u64 | HHDM_OFFSET as u64) as *mut PageTable;
            
                for i in 0..256 {
                    let p3_entry = &(*pml4_virt).entries[i];
                    if !p3_entry.is_present() { continue; }
                    let p3 = (p3_entry.get_addr().as_u64() | HHDM_OFFSET as u64) as *mut PageTable;
                
                    for j in 0..512 {
                        let p2_entry = &(*p3).entries[j];
                        if !p2_entry.is_present() { continue; }
                        let p2 = (p2_entry.get_addr().as_u64() | HHDM_OFFSET as u64) as *mut PageTable;
                    
                        for k in 0..512 {
                            let p1_entry = &(*p2).entries[k];
                            if !p1_entry.is_present() { continue; }
                            let _p1 = (p1_entry.get_addr().as_u64() | HHDM_OFFSET as u64) as *mut PageTable;
                            FrameAllocator::free_frame((*p2).entries[k].get_addr());
                        }
                    
                        FrameAllocator::free_frame((*p3).entries[j].get_addr());
                    }
                
                    FrameAllocator::free_frame((*pml4_virt).entries[i].get_addr());
                }
            
                FrameAllocator::free_frame(PhysAddr::new(process.pml4 as u64));
            }
...
#[unsafe(no_mangle)]
fn process_exit(return_value: Option<i64>) {
    unsafe {
        let current = PROCESS_TABLE.current;
        if let Some(proc) = PROCESS_TABLE.processes[current].as_mut() {
            println!("Process {} exited\nReturn val: {:?}", proc.pid, return_value);
            proc.ret_value = return_value;
            proc.state = ProcessState::Terminated;
        }
