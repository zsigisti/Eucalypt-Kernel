    ...
    pub fn load_from_fat12(&self, image: &[u8]) -> Result<(), VfsError> {
        ...
        for i in 0..bpb.root_entry_count as usize {
            ...
            while cluster >= 2 && cluster < FAT12_BAD {
                ...
                let next = if cluster & 1 == 0 { val & 0x0FFF } else { val >> 4 };
...
impl FileSystem for RamFs {
    fn stat(&self, path: &str) -> Result<FileStat, VfsError> {
        ...
        if path == "." || path == "" || inner.dirs.iter().any(|d| d.path == path) {
    ...
    fn readdir(&self, path: &str) -> Result<Vec<DirEntry>, VfsError> {
        ...
        for f in &inner.files {
            ...
            } else if f.path.starts_with(path) && f.path.len() > path.len() {
                let rel = f.path[path.len()..].trim_start_matches('/');
        ...
        for d in &inner.dirs {
            ...
            } else if d.path.starts_with(path) && d.path.len() > path.len() {
                let rel = d.path[path.len()..].trim_start_matches('/');
                if !rel.contains('/') && !rel.is_empty() {
