...
fn main() {
    ...
    // kernel.map is generated by the second Makefile pass (nm -n kernel | grep " [Tt] ").
    // On the first pass it doesn't exist yet, so create an empty placeholder so that
    // include_str!("../kernel.map") in main.rs doesn't fail the build.
    if !std::path::Path::new("kernel.map").exists() {
        let _ = std::fs::write("kernel.map", "");
    }
